...
class StoneAnalysisApp:
    def __init__(self):
        ...
        self.colormaps = {
            ...
            #'earth_tones': ['#000000', '#8B4513', '#A0522D', '#CD853F', '#D2691E', '#DEB887', '#F4A460', '#FFEAA7', '#FFE135', '#FFF700', '#FFFACD', '#FFFFFF'],
            'thermal': ['#000080', '#000099', '#0000FF', '#0066FF', '#00CCFF', '#00FFCC', '#66FF66', '#CCFF00', '#FFCC00', '#FF6600', '#FF0000', '#FFFFFF'],
            #'rainbow': ['#000000', '#4B0082', '#0000FF', '#0080FF', '#00FFFF', '#00FF80', '#00FF00', '#80FF00', '#FFFF00', '#FF8000', '#FF0000', '#FFFFFF'],
            #'pastel_bold': ['#000000', '#6B46C1', '#3B82F6', '#06B6D4', '#10B981', '#84CC16', '#EAB308', '#F59E0B', '#EF4444', '#EC4899', '#A855F7', '#FFFFFF'],
            #'medical': ['#000000', '#1A1A2E', '#16213E', '#0F3460', '#533483', '#7209B7', '#A663CC', '#4ECDC4', '#45B7D1', '#96CEB4', '#FECA57', '#FFFFFF'],
            'geological': ['#000000', '#2C1810', '#4A2C2A', '#6B4423', '#8B5A2B', '#CD853F', '#DAA520', '#F0E68C', '#FFFACD', '#F5F5DC', '#FFFFF0', '#FFFFFF'],
            'cyberpunk': self.create_cyberpunk_colormap(),
            'neon': self.create_neon_colormap(),
            'matrix': self.create_matrix_colormap(),
            'ultra_high_contrast': self.create_ultra_contrast_colormap()
        }
    ...
    def setup_intensity_density_mapping(self):
        ...
        # For better whewellite detection, analyze the actual intensity distribution
        stone_50th = np.percentile(stone_intensities, 50)
        stone_75th = np.percentile(stone_intensities, 75)
        stone_90th = np.percentile(stone_intensities, 90)
        stone_95th = np.percentile(stone_intensities, 95)
        stone_99th = np.percentile(stone_intensities, 99)
        print(f"📊 Stone intensity distribution:")
        print(f"    Min: {stone_min:.0f} HU")
        print(f"    50th percentile: {stone_50th:.0f} HU") 
        print(f"    75th percentile: {stone_75th:.0f} HU")
        print(f"    90th percentile: {stone_90th:.0f} HU")
        print(f"    95th percentile: {stone_95th:.0f} HU")
        print(f"    99th percentile: {stone_99th:.0f} HU")
        print(f"    Max: {stone_max:.0f} HU")
        
        # Create calibration curve using actual percentile-based thresholds
        # This ensures the mapping corresponds to real intensity distributions
        intensity_points = np.array([
            self.background_ct_intensity,  # Background/air
            stone_min,     # 100% bacteria (minimum stone intensity)
            stone_50th,    # 90% bacteria (median stone intensity)
            stone_75th,    # 70% bacteria (75th percentile)
            stone_90th,    # 40% bacteria (90th percentile) 
            stone_95th,    # 20% bacteria (95th percentile)
            stone_99th,    # 10% bacteria (99th percentile)
            stone_max      # Pure whewellite (maximum intensity)
        ])
        
        density_points = np.array([
            0.001,  # Air density
            0.95,   # 100% bacteria (minimum intensity)
            1.05,   # 90% bacteria (50th percentile)
            1.25,   # 70% bacteria (75th percentile)
            1.55,   # 40% bacteria (90th percentile)
            1.75,   # 20% bacteria (95th percentile)
            1.95,   # 10% bacteria (99th percentile)
            2.23    # Pure whewellite (maximum intensity)
        ])
        ...
        comp_names = ['Air', 'Min (100% Bacteria)', '50th %ile (90% Bacteria)', '75th %ile (70% Bacteria)', '90th %ile (40% Bacteria)', '95th %ile (20% Bacteria)', '99th %ile (10% Bacteria)', 'Max (Pure Whewellite)']
    ...
    def create_main_window(self):
        ...
        # Composition zones (clickable) - ENHANCED CONTRAST with advanced processing
        colors = self.colormaps[self.current_colormap]
        cmap_zones = ListedColormap(colors)
        self.comp_ax = self.axes[4]
        
        # Apply enhancement based on colormap type (temporarily simplified for debugging)
        if self.current_colormap in ['cyberpunk', 'neon', 'matrix', 'ultra_high_contrast']:
            # Temporarily use simple display to debug the black colormap issue
            print(f"🎨 Displaying {self.current_colormap} colormap with {len(colors)} colors")
            print(f"🔍 Composition map range: {np.min(self.composition_map)} to {np.max(self.composition_map)}")
            self.comp_ax.imshow(self.composition_map, cmap=cmap_zones, vmin=0, vmax=11, interpolation='bilinear')
            self.comp_ax.set_title(f'Composition Zones - {self.current_colormap.title()} Style (Click for Line Scan)')
        elif self.current_colormap == 'morphological':
            ...
            # Apply spline smoothing for better quality
            smoothed_map = self.apply_spline_smoothing(self.composition_map, smoothing_factor=1.5)
            self.comp_ax.imshow(smoothed_map, cmap=cmap_zones, vmin=0, vmax=11, interpolation='bilinear')
            self.comp_ax.set_title('Composition Zones - Smoothed (Click for Line Scan)')
        
        self.comp_ax.axis('off')
        
        # Overlay - Enhanced contrast overlay with advanced processing
        ...
        # Apply enhancement to overlay based on colormap type
        if self.current_colormap in ['cyberpunk', 'neon', 'matrix', 'ultra_high_contrast']:
            enhanced_overlay = self.create_high_resolution_map(overlay_map, scale_factor=2)
            self.axes[5].imshow(enhanced_overlay, cmap=cmap_zones, vmin=0, vmax=11, alpha=0.9, interpolation='bicubic')
        elif self.current_colormap == 'morphological':
            ...
            smoothed_overlay = self.apply_spline_smoothing(overlay_map, smoothing_factor=1.0)
            self.axes[5].imshow(smoothed_overlay, cmap=cmap_zones, vmin=0, vmax=11, alpha=0.7, interpolation='bilinear')
...
Colormap: {self.current_colormap}"""
    ...
    def cycle_colormap(self, event):
        ...
        elif self.current_colormap in ['cyberpunk', 'neon', 'matrix', 'ultra_high_contrast']:
            print(f"   🚀 High-resolution {self.current_colormap} style activated - Enhanced smoothing & bicubic interpolation!")
        else:
            print("   🎯 Spline smoothing active - Improved image quality enabled!")
    ...
    def update_displays(self):
        ...
        if self.current_colormap in ['cyberpunk', 'neon', 'matrix', 'ultra_high_contrast']:
            # Temporarily use simple display to debug the black colormap issue
            print(f"🎨 Updating {self.current_colormap} colormap with {len(colors)} colors")
            self.comp_ax.imshow(self.composition_map, cmap=cmap_zones, vmin=0, vmax=11, interpolation='bilinear')
            self.comp_ax.set_title(f'Composition Zones - {self.current_colormap.title()} Style (Click for Line Scan)')
        elif self.current_colormap == 'morphological':
            enhanced_comp_map = self.enhance_composition_contrast(self.composition_map)
...
Colormap: {self.current_colormap}"""
    ...
    def extract_and_display_line_scan(self):
        ...
        # Store the calculated profiles for consistent reuse
        ...
        self.line_scan_intensity = intensity_profile  # Store intensity profile for consistent calculations
        
        # Group zones for cleaner line profile analysis (using stored data)
        grouped_profile, grouped_densities = self.group_zones_for_line_profile_cached(composition_profile, density_profile)
        
        print(f"📊 Line scan extracted: {len(composition_profile)} pixels")
        print(f"📊 Using actual density profile calculated from CT intensity")
    ...
    def create_line_scan_window(self):
        ...
        # 2. Enhanced Density profile with fine gradation but zone references
        # Define density thresholds for composition region references
        air_density = 0.001
        ...
        # Horizontal threshold lines only (no background shading)
        ...
        # Add composition range labels inside the graph on the left side
        # Use actual data coordinates for proper positioning
        x_range = ax2.get_xlim()
        x_label_pos = x_range[0] + (x_range[1] - x_range[0]) * 0.02  # 2% from left edge
        
        ax2.text(x_label_pos, 0.75, '>90% B', 
                ha='left', va='center', fontsize=8, fontweight='bold', color='black', 
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        ax2.text(x_label_pos, (bacteria_density + bacteria_rich_density) / 2, '60-90% B', 
                ha='left', va='center', fontsize=8, fontweight='bold', color='black', 
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        ax2.text(x_label_pos, (bacteria_rich_density + intergrowth_density) / 2, '40-60% B', 
                ha='left', va='center', fontsize=8, fontweight='bold', color='black',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        ax2.text(x_label_pos, (intergrowth_density + whewellite_rich_density) / 2, '10-40% B', 
                ha='left', va='center', fontsize=8, fontweight='bold', color='black',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        ax2.text(x_label_pos, (whewellite_rich_density + pure_whewellite_density) / 2, '<10% B', 
                ha='left', va='center', fontsize=8, fontweight='bold', color='black',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        
        # Use the already calculated density profile from line scan data
        fine_density_profile = self.line_scan_densities
        
        # Plot fine-grained density profile
        ax2.plot(x_pixels, fine_density_profile, 'o-', linewidth=2, markersize=3, color='red')
        ax2.fill_between(x_pixels, fine_density_profile, alpha=0.3, color='red')
        
        # Calculate statistics from fine-grained data
        density_min, density_max = fine_density_profile.min(), fine_density_profile.max()
        
        # Calculate weighted mean using the same method as terminal output
        grouped_profile, grouped_densities = self.group_zones_for_line_profile(self.line_scan_data)
        stone_pixels = grouped_profile > 0  # Exclude background
        if np.sum(stone_pixels) > 0:
            stone_densities = fine_density_profile[stone_pixels]
            
            # Calculate composition-weighted mean using actual density values (same as terminal)
            weighted_sum = 0
            total_weight = 0
            for group_val in [1, 2, 3, 4, 5]:
                group_mask = grouped_profile == group_val
                if np.sum(group_mask) > 0:
                    # Use actual density values for this group (same calculation as terminal)
                    group_densities = grouped_densities[group_mask]
                    group_median_density = np.median(group_densities)
                    count = np.sum(group_mask)
                    weight = count / np.sum(stone_pixels)
                    weighted_sum += weight * group_median_density
                    total_weight += weight
            
            display_mean = weighted_sum if total_weight > 0 else np.median(stone_densities)
        else:
            display_mean = fine_density_profile.mean()
        
        ax2.axhline(display_mean, color='black', linestyle='-', alpha=0.8, linewidth=2,
                   label=f'Weighted Mean: {display_mean:.2f} g/cm³')
        ...
        # Remove legend since we now use text labels instead of colored regions
        ...
        # 4. Statistics horizontal bar chart - Show fine detail zones matching composition profile
        # Use the actual fine composition zones instead of grouped categories
        composition_labels = ['Background', '100% Bacteria', '90% Bacteria', '80% Bacteria', '70% Bacteria', '60% Bacteria', 
                            '50/50 Mix', '40% Bacteria', '30% Bacteria', '20% Bacteria', '10% Bacteria', 'Pure Whewellite', 'Holes/Voids']
        unique_values, counts = np.unique(self.line_scan_data, return_counts=True)
        
        # Filter out background (value 0) and holes/voids (value 12) from chart calculations
        labels = []
        percentages = []
        colors_bar = []
        ...
        for val, count in zip(unique_values, counts):
            if val > 0 and val < 12 and count > 0:  # Exclude background (val=0) and holes/voids (val=12)
                total_stone_only += count
        
        # Use the same colormap as the composition profile
        
        for val, count in zip(unique_values, counts):
            if val > 0 and val < 12 and count > 0:  # Exclude background (val=0) and holes/voids (val=12)
                if val < len(composition_labels):
                    name = composition_labels[int(val)]
                    # Get density from component densities
                    density_key_map = {
                        1: '100% Bacteria',
                        2: '90% Bacteria', 
                        3: '80% Bacteria',
                    ...
                    if val in density_key_map and density_key_map[val] in self.component_densities:
                        actual_density = self.component_densities[density_key_map[val]]
                    else:
                        actual_density = 1.50  # Fallback density
                    
                    pct = count/total_stone_only*100 if total_stone_only > 0 else 0
                    labels.append(f'{name}\n({actual_density:.2f} g/cm³)')
                    percentages.append(pct)
                    
                    # Use the same color as in the composition profile
                    if int(val) < len(colors):
                        colors_bar.append(colors[int(val)])
                    else:
                        colors_bar.append('#888888')  # Gray fallback
        ...
        if percentages:
            ...
            ax4.set_title('Stone Composition\n(Fine Detail Zones)', fontsize=12, fontweight='bold')
    ...
    def print_line_scan_statistics(self):
        ...
        # Calculate fine-grained density profile for statistics
        intensity_profile = measure.profile_line(
            self.ct_image, self.line_start, self.line_end,
            linewidth=1, mode='constant'
        )
        
        # Apply smoothing for consistent results
        from scipy import ndimage
        smoothed_intensity = ndimage.gaussian_filter1d(intensity_profile, sigma=2.0)
        fine_density_profile = self.intensity_to_density(smoothed_intensity)
        fine_density_profile = np.maximum(fine_density_profile, 0)
        ...
        # Calculate weighted statistics for stone material only
        grouped_profile, grouped_densities = self.group_zones_for_line_profile(self.line_scan_data)
        stone_pixels = grouped_profile > 0  # Exclude background
        ...
        if np.sum(stone_pixels) > 0:
            ...
            # Calculate composition-weighted mean using actual density values
            weighted_sum = 0
            total_weight = 0
            print(f"\n🔍 Weighted Mean Calculation Debug:")
            for group_val in [1, 2, 3, 4, 5]:
                group_mask = grouped_profile == group_val
                if np.sum(group_mask) > 0:
                    # Use actual density values for this group
                    group_densities = grouped_densities[group_mask]
                    # Use median instead of mean to reduce outlier influence
                    group_median_density = np.median(group_densities)
                    group_mean_density = np.mean(group_densities)
                    count = np.sum(group_mask)
                    weight = count / np.sum(stone_pixels)
                    weighted_sum += weight * group_median_density
                    total_weight += weight
                    
                    group_names = ['', 'Pure Bacteria', 'Bacteria-Rich', 'Intergrowth', 'Whewellite-Rich', 'Pure Whewellite']
                    print(f"  {group_names[group_val]:15s}: {count:4d} pixels ({weight*100:5.1f}%) - Mean: {group_mean_density:.3f}, Median: {group_median_density:.3f} g/cm³")
            
            composition_weighted_mean = weighted_sum if total_weight > 0 else np.median(stone_densities)
            print(f"  Final weighted mean: {composition_weighted_mean:.3f} g/cm³")
            
            print(f"Density range: {stone_densities.min():.3f} - {stone_densities.max():.3f} g/cm³")
            print(f"Arithmetic mean density: {np.mean(stone_densities):.3f} ± {np.std(stone_densities):.3f} g/cm³")
            print(f"Composition-weighted mean: {composition_weighted_mean:.3f} g/cm³")
            print(f"Median density: {np.median(stone_densities):.3f} g/cm³")
        else:
            print(f"Density range: {fine_density_profile.min():.3f} - {fine_density_profile.max():.3f} g/cm³")
            print(f"Mean density: {fine_density_profile.mean():.3f} ± {fine_density_profile.std():.3f} g/cm³")
            print(f"Median density: {np.median(fine_density_profile):.3f} g/cm³")
    ...
    def export_data(self, event):
        ...
        # Calculate fine-grained density profile for export
        from scipy import ndimage
        smoothed_intensity = ndimage.gaussian_filter1d(intensity_profile, sigma=2.0)
        fine_density_profile = self.intensity_to_density(smoothed_intensity)
        fine_density_profile = np.maximum(fine_density_profile, 0)
        ...
        export_df = pd.DataFrame({
            ...
            'Density_g_cm3': fine_density_profile,
        ...
        # Calculate composition-weighted mean for export
        grouped_profile, grouped_densities = self.group_zones_for_line_profile(self.line_scan_data)
        stone_pixels = grouped_profile > 0
        ...
        if np.sum(stone_pixels) > 0:
            ...
            for group_val in [1, 2, 3, 4, 5]:
                ...
                if np.sum(group_mask) > 0:
                    ...
                    weighted_sum += weight * group_median_density
                    total_weight += weight
            composition_weighted_mean = weighted_sum if total_weight > 0 else np.median(fine_density_profile)
        else:
            composition_weighted_mean = np.median(fine_density_profile)
        
        print(f"   Density range: {fine_density_profile.min():.3f} - {fine_density_profile.max():.3f} g/cm³")
        print(f"   Arithmetic mean: {fine_density_profile.mean():.3f} g/cm³")
        print(f"   Composition-weighted mean: {composition_weighted_mean:.3f} g/cm³")
        ...
        metadata = {
            ...
            'density_min': float(fine_density_profile.min()),
            'density_max': float(fine_density_profile.max()),
            'density_mean': float(fine_density_profile.mean()),
            'density_std': float(fine_density_profile.std()),
            'composition_weighted_mean': float(composition_weighted_mean),
            'analysis_parameters': {
    ...
    def group_zones_for_line_profile_cached(self, composition_profile, density_profile):
        """Group fine composition zones into broader categories using pre-calculated density values"""
        ...
        for i, zone in enumerate(composition_profile):
            zone = int(zone)
            actual_density = density_profile[i]
            
            # Respect the visual composition zones and use actual density values
            if zone == 0:  # Background
                grouped_profile[i] = 0
                grouped_densities[i] = 0.001
            elif zone == 12:  # Holes/Voids
                grouped_profile[i] = 6
                grouped_densities[i] = 0.001
            elif zone in [1, 2]:  # Pure Bacteria group (zones 1-2: 100%, 90% bacteria)
                grouped_profile[i] = 1
                grouped_densities[i] = actual_density
            elif zone in [3, 4]:  # Bacteria-Rich group (zones 3-4: 80%, 70% bacteria)
                grouped_profile[i] = 2
                grouped_densities[i] = actual_density
            elif zone in [5, 6]:  # Intergrowth group (zones 5-6: 60%, 50% bacteria)
                grouped_profile[i] = 3
                grouped_densities[i] = actual_density
            elif zone in [7, 8]:  # Whewellite-Rich group (zones 7-8: 40%, 30% bacteria)
                grouped_profile[i] = 4
                grouped_densities[i] = actual_density
            elif zone in [9, 10, 11]:  # Pure Whewellite group (zones 9-11: 20%, 10%, 0% bacteria)
                grouped_profile[i] = 5
                grouped_densities[i] = actual_density
            else:  # Default fallback
                grouped_profile[i] = 3  # Default to intergrowth
                grouped_densities[i] = actual_density
        
        return grouped_profile, grouped_densities

    def group_zones_for_line_profile(self, composition_profile):
        """Group fine composition zones into broader categories while respecting visual composition zones"""
        # Use cached values if available, otherwise calculate fresh
        if hasattr(self, 'line_scan_densities') and len(self.line_scan_densities) == len(composition_profile):
            return self.group_zones_for_line_profile_cached(composition_profile, self.line_scan_densities)
        
        # Fallback: Calculate fresh (for backward compatibility)
        # Calculate actual density values for each pixel in the line scan
        intensity_profile = measure.profile_line(
            self.ct_image, self.line_start, self.line_end,
            linewidth=1, mode='constant'
        )
        ...
        return self.group_zones_for_line_profile_cached(composition_profile, fine_density_profile)
    ...
    def apply_spline_smoothing(self, composition_map, smoothing_factor=2.0):
        """Apply spline smoothing for higher quality composition maps"""
        from scipy import ndimage
        from skimage import transform
        
        # Convert to float for processing
        float_map = composition_map.astype(np.float32)
        
        # Check if the input map has valid data
        if np.max(float_map) == 0:
            print("⚠️ Warning: Composition map is empty, returning original")
            return composition_map
        
        # Apply bilateral filter to preserve edges while smoothing
        try:
            from skimage.restoration import denoise_bilateral
            bilateral_available = True
        except ImportError:
            bilateral_available = False
        
        # Smooth each zone individually to preserve boundaries
        smoothed_map = np.zeros_like(float_map)
        
        for zone in range(1, 12):  # For each composition zone
            zone_mask = (composition_map == zone)
            if np.sum(zone_mask) > 0:
                # Create temporary image for this zone
                zone_image = np.zeros_like(float_map)
                zone_image[zone_mask] = zone
                
                # Apply Gaussian smoothing with edge preservation
                if np.sum(zone_mask) > 100:  # Only smooth if zone is large enough
                    smoothed_zone = ndimage.gaussian_filter(zone_image, sigma=smoothing_factor/2)
                    # Preserve zone boundaries by only applying smoothing within the zone
                    smoothed_map[zone_mask] = smoothed_zone[zone_mask]
                else:
                    smoothed_map[zone_mask] = zone
        
        # Apply subtle edge-preserving filter to the entire map
        if bilateral_available:
            try:
                smoothed_map = denoise_bilateral(smoothed_map, sigma_color=0.1, sigma_spatial=smoothing_factor)
            except:
                # Fallback to Gaussian if bilateral fails
                smoothed_map = ndimage.gaussian_filter(smoothed_map, sigma=smoothing_factor/3)
        else:
            # Fallback to Gaussian if bilateral not available
            smoothed_map = ndimage.gaussian_filter(smoothed_map, sigma=smoothing_factor/3)
        
        # Safety check: ensure we haven't lost all the data
        if np.max(smoothed_map) == 0:
            print("⚠️ Warning: Smoothing resulted in empty map, returning original")
            return composition_map
        
        return smoothed_map
    
    def create_high_resolution_map(self, composition_map, scale_factor=2):
        """Create higher resolution version of composition map using interpolation"""
        from skimage import transform
        
        # Check if the input map has valid data
        if np.max(composition_map) == 0:
            print("⚠️ Warning: Composition map is empty, returning original")
            return composition_map
        
        try:
            # Upscale using cubic interpolation for smooth results
            high_res_map = transform.rescale(
                composition_map.astype(np.float32), 
                scale_factor, 
            ...
            # Apply light spline smoothing to the high-resolution map
            smoothed_high_res = self.apply_spline_smoothing(high_res_map, smoothing_factor=0.8)
            
            # Downscale back to original size with improved quality
            final_map = transform.rescale(
                smoothed_high_res,
                1.0/scale_factor,
                order=3,
                preserve_range=True,
            ...
            if np.max(final_map) == 0:
                print("⚠️ Warning: High-resolution processing resulted in empty map, returning original")
                return composition_map
            
            return final_map
            
        except Exception as e:
            print(f"⚠️ Warning: High-resolution processing failed ({e}), returning original")
            return composition_map
    ...
    def create_cyberpunk_colormap(self):
        """Create cyberpunk-style high contrast colormap with neon colors"""
        cyberpunk_colors = [
            '#000000',  # Background - Pure Black
            '#FF006E',  # Zone 1 - Hot Pink (100% Bacteria)
            '#FB5607',  # Zone 2 - Electric Orange (90% Bacteria)
            '#FFBE0B',  # Zone 3 - Cyber Yellow (80% Bacteria)
            '#8338EC',  # Zone 4 - Electric Purple (70% Bacteria)
            '#3A86FF',  # Zone 5 - Neon Blue (60% Bacteria)
            '#06FFA5',  # Zone 6 - Electric Green (50/50 Mix)
            '#FF4081',  # Zone 7 - Neon Pink (40% Bacteria)
            '#00E5FF',  # Zone 8 - Cyan (30% Bacteria)
            '#B388FF',  # Zone 9 - Light Purple (20% Bacteria)
            '#FF5722',  # Zone 10 - Electric Red (10% Bacteria)
            '#FFFF00',  # Zone 11 - Pure Yellow (Pure Whewellite)
            '#FFFFFF'   # Zone 12 - White (Holes/Voids)
        ]
        print(f"🎨 Created cyberpunk colormap with {len(cyberpunk_colors)} colors")
        return cyberpunk_colors
    
    def create_neon_colormap(self):
        """Create neon-style colormap with glowing effects"""
        neon_colors = [
            '#000000',  # Background - Black
            '#FF1744',  # Zone 1 - Neon Red (100% Bacteria)
            '#FF9100',  # Zone 2 - Neon Orange (90% Bacteria)
            '#FFEA00',  # Zone 3 - Neon Yellow (80% Bacteria)
            '#76FF03',  # Zone 4 - Neon Green (70% Bacteria)
            '#00E676',  # Zone 5 - Bright Green (60% Bacteria)
            '#00BCD4',  # Zone 6 - Neon Cyan (50/50 Mix)
            '#2196F3',  # Zone 7 - Electric Blue (40% Bacteria)
            '#3F51B5',  # Zone 8 - Neon Indigo (30% Bacteria)
            '#9C27B0',  # Zone 9 - Electric Purple (20% Bacteria)
            '#E91E63',  # Zone 10 - Hot Pink (10% Bacteria)
            '#FFF176',  # Zone 11 - Bright Yellow (Pure Whewellite)
            '#FFFFFF'   # Zone 12 - White (Holes/Voids)
        ]
        print(f"🎨 Created neon colormap with {len(neon_colors)} colors")
        return neon_colors
    
    def create_matrix_colormap(self):
        """Create Matrix-style green colormap"""
        matrix_colors = [
            '#000000',  # Background - Black
            '#001100',  # Zone 1 - Very Dark Green (100% Bacteria)
            '#003300',  # Zone 2 - Dark Green (90% Bacteria)
            '#004400',  # Zone 3 - Medium Dark Green (80% Bacteria)
            '#006600',  # Zone 4 - Green (70% Bacteria)
            '#008800',  # Zone 5 - Bright Green (60% Bacteria)
            '#00AA00',  # Zone 6 - Light Green (50/50 Mix)
            '#00CC00',  # Zone 7 - Brighter Green (40% Bacteria)
            '#00FF00',  # Zone 8 - Neon Green (30% Bacteria)
            '#33FF33',  # Zone 9 - Light Neon Green (20% Bacteria)
            '#66FF66',  # Zone 10 - Pale Green (10% Bacteria)
            '#99FF99',  # Zone 11 - Very Light Green (Pure Whewellite)
            '#FFFFFF'   # Zone 12 - White (Holes/Voids)
        ]
        print(f"🎨 Created matrix colormap with {len(matrix_colors)} colors")
        return matrix_colors
    
    def create_ultra_contrast_colormap(self):
        """Create ultra high contrast colormap with maximum distinction"""
        ultra_colors = [
            '#000000',  # Background - Black
            '#FF0000',  # Zone 1 - Pure Red (100% Bacteria)
            '#00FF00',  # Zone 2 - Pure Green (90% Bacteria)
            '#0000FF',  # Zone 3 - Pure Blue (80% Bacteria)
            '#FFFF00',  # Zone 4 - Pure Yellow (70% Bacteria)
            '#FF00FF',  # Zone 5 - Pure Magenta (60% Bacteria)
            '#00FFFF',  # Zone 6 - Pure Cyan (50/50 Mix)
            '#FF8000',  # Zone 7 - Orange (40% Bacteria)
            '#8000FF',  # Zone 8 - Purple (30% Bacteria)
            '#FF0080',  # Zone 9 - Pink (20% Bacteria)
            '#80FF00',  # Zone 10 - Lime (10% Bacteria)
            '#0080FF',  # Zone 11 - Sky Blue (Pure Whewellite)
            '#FFFFFF'   # Zone 12 - White (Holes/Voids)
        ]
        print(f"🎨 Created ultra contrast colormap with {len(ultra_colors)} colors")
        return ultra_colors
